...
public class DispatcherServlet extends HttpServlet {
	...
	private ViewResolver viewResoler;


	public void init(ServletConfig config) throws ServletException {
		super.init();
		hadlerMapping = new HandlerMapping();
		viewResoler = new ViewResolver();
		viewResoler.setPrefix("./"); //파일의 위치경로(현재위치)
		viewResoler.setSuffix(".jsp"); //파일의 타입(속성)
	}
...
*/		
		...
		//ViewResolver 를 사용해서 응답할 페이지 정보 만들기
		String view = null;
		if (viewName.contains(".do")) {
			view = viewName;
		} else { //뷰(View) 이름만 전달받은 경우
			view = viewResoler.getView(viewName);
		}
		System.out.println("view 정보 : " + view);
